					...
					if (!data1.caster().getWorld().isClient) {
						if (!data1.targets().isEmpty()) {
							...
							for (Entity target : data1.targets()) {
								...
								((WorldScheduler)data1.caster().getWorld()).schedule(1+2*ii,()->{
									data1.caster().lookAt(EntityAnchorArgumentType.EntityAnchor.EYES,target.getBoundingBox().getCenter());
								SpellHelper.performSpell(data1.caster().getWorld(), data1.caster(), Identifier.of(MODID,"eldritch_blasts"), (List.of(target)), SpellCast.Action.RELEASE,1.0F);
								});
								ii++;
							}
						}
